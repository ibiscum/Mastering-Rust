...
mod tests {
    ...
    fn bytestring_to_string_with_match_uppercases_valid_utf8() {
        let bytes = vec!(80, 82, 84, 85, 86); // PRTUV
    ...
    #[test]
    fn bytestring_to_string_uppercases_valid_utf8() {
        let bytes = vec!(80, 82, 84, 85, 86);
        assert_eq!(bytestring_to_string(bytes).unwrap(), "PRTUV");
    }

    #[test]
    fn bytestring_to_string_returns_err_for_invalid_utf8() {
        let bytes = vec!(130, 131, 132, 133);
        assert!(bytestring_to_string(bytes).is_err());
    }
}
